...
## 2026-09-25 — 共用既有模組會改到別的功能時，先講清楚再動手

LEAPS 垂直價差規格寫「沿用 bcvs 的快取表與 sidecar」。我照做，順手改了 bcvs 三處
（快取篩選時點、到期日 sidecar 狀態、job 加鎖），雖然都有測試保證 bcvs 行為不變，
使用者事後問「為什麼會動到 bcvs？」並裁示完全分開——等於白做一輪、還要還原。

- P0 的待決事項雖然問過「篩選移到讀取」，但問法是技術選項，沒有講明
  「這代表 bcvs 的程式和資料會被修改」。使用者選的是方案，不是同意動 bcvs。
- **規則**：任何會修改其他功能（別的頁面、別的 job、別的表）的做法，提問時第一句就寫
  「這會改到 ○○ 功能的哪些地方」，並把「完全分開」列為選項之一。
- 還原時連資料一起還原：P1 首版寫進 bcvs 表的 ORCL 資料也要刪，不只是程式碼。

## 2026-09-25 — 健康檢查的「活著」≠ 能用；提方案前先查清楚功能的正確用法

ORCL 報「No Chrome CDP page found」：Chrome 程序活著、`/json/version` 回 200，但 0 個分頁。

### 教訓 A：存活檢查要檢查「消費者真正需要的東西」

keeper 和 `CdpPrecheckable` 都只問「瀏覽器程序在不在」，爬蟲需要的卻是「有沒有分頁可用」。
兩道防線看的都是同一個錯的指標，所以同時放行。設計健康檢查時，先問下游實際拿什麼來用。

### 教訓 B：守護程式要確認自己的迴圈真的在跑

keeper 前景呼叫 chrome.exe，卡在那一行 26 小時，看起來 `online`、重啟次數 0，
其實一次都沒檢查。查 keeper 類問題第一步：`ps --ppid $(pm2 pid <name>)` 看它正在做什麼。

### 教訓 C：提出 Chrome 參數／功能前先查原始碼並實測

我一開始說 `--window-name`「還沒找到官方文件確認」就把它列進方案，被使用者糾正要先查清楚。
正確做法：`gh search code '"window-name"' --repo chromium/chromium` 找到 `chrome_switches.h`
的定義與使用點，再對真實 Chrome 實測（連「轉交給既有程序開的新視窗也會套用」都要測），
確認後才寫進方案。

## 2026-09-09 — 今天的主線：不確定卻不問，用「改一版給你看」代替問一句

Price-In 的「兩顆帶入按鈕各自該寫哪些欄位、綠帶用哪一年」這一個問題，
**連續改錯五次**。使用者每次都得截圖標紅箭頭才能糾正。

| 次 | 我做了 | 錯在哪 |
|---|---|---|
| 1 | 兩顆都寫圖 A | 沒問就決定 |
| 2 | 本年度→圖A、下一年度→圖B | 半對 |
| 3 | 「選哪一年整張圖就那一年」 | 自作主張改語意 |
| 4 | 加第二條綠帶 | 使用者沒要求的東西 |
| 5 | 本年度→年度＋綠帶 | 綠帶被洗掉的根源 |

中途用過一次 AskUserQuestion，**問完拿到答案又自己改成別的**，等於白問。

### 教訓 A：改一版的成本是問一句的十倍以上

```
問一句：          約 1 分鐘
改一版給對方看：  編輯 → vite build → assets:precompile → pm2 restart
                  → 對方重新整理 → 截圖 → 回報   ≈ 10–15 分鐘
```

在這個專案裡「試試看」特別貴，因為每次驗證都要走完整條建置鏈。

```
同一個問題猜錯一次就停下來問；問完照答案做，不要「順手優化」。
覺得答案有問題就說出來讓對方決定，不要自己改掉。
```

### 教訓 B：使用者說「壞了」時，不要推測他的操作

我說過「你沒按重新出圖吧」——對方按了。真正原因是我的 `force` 防護
擋住了寫入。**用推測質疑使用者，等於把自己的 bug 推給他。**

```
在有證據之前不推測對方的操作。要驗證就自己去驗證。
```

### 教訓 C：有 Playwright MCP 就不要讓人肉截圖

因為前一天「劫持使用者分頁」的教訓，這次矯枉過正，整天靠對方截圖。
**對方登入自動化 Chrome 之後，三分鐘內就定位到問題**——那三分鐘
本來可以是三小時前的事。

另外發現：MCP 控制的就是使用者正在操作的那個視窗
（網址帶著他剛填的 `entry_b_price=150`），雙方一直互相覆蓋狀態。

```
需要看畫面時先問「我可以用瀏覽器嗎」，不是預設不用。
要操作前先講一聲，避免和對方在同一個分頁上打架。
```

### 教訓 D：系統性問題要一次掃完整條路徑

期權小學堂的 CSP 有三層破壞（內嵌樣式 → 內嵌 `<script>` → `onclick`
事件屬性），我修一層等對方回報一次，等於讓他替我做三次測試。

第三層的關鍵是 **nonce 對「屬性」無效，只對「區塊」有效**——
`style="..."` 與 `onclick="..."` 都救不了。第二層我用注入 nonce 修好了
`<script>`，卻救不了 `onclick`，這件事在修第二層時就該知道。

```
修 CSP／權限／認證這類系統性問題，先把整條路徑掃完再回報。
```

### 教訓 E：規格是參考，使用者的指示優先

「圖 A 的年度必須一致」是規格寫的，所以每次看到 FY2026 配 FY2027
就想「修正」它——但那正是使用者要的對照（今年的門檻 vs 明年的預期）。

```
規格與使用者要求衝突時，照使用者的做，然後 patch 規格說明為什麼改。
```

### 教訓 F：不要加沒被要求的東西

第二條綠帶、逐步導覽按鈕、「年度錯置」警告都是自己判斷「應該要有」
就加，然後又整個撤掉。**撤掉的成本也是對方的時間。**

```
只做被要求的。想到的改進先講不做。
```

### 教訓 G：Rails 專案裡不要用 Python 當編輯工具

滿螢幕 `python3 - <<'PY'` 讓使用者看不出到底改了什麼，
即使沒有任何 `.py` 進 repo，觀感與可稽核性都很差。

```
改檔一律用 Edit／Write 工具（有 before/after diff）。
python 只用於一次性的算術驗證。
```

### 教訓 H：加新元件前，先找專案裡的同類元件

新增 `PriceIn::TutorialComponent`（頁底教學）時，把當天討論 NOK 的股價、EPS、
倍數**寫死在 locale 當教材**。使用者一句話點破：

> 「這教學的式子是固定的，應依使用者輸入的美股代號及數值來做推算，
> 不應寫成固定的 nok 固定的價格」

同專案的 `PriceIn::LogicExampleBuilder` 類別註解第一行就寫著
「用使用者當前填的數字，演一次」——判讀卡下面那張表就是它畫的。
我加新元件時沒去看它。

這不是需求不清楚，是沒做功課，而且後果比「不夠貼心」嚴重：Price-In 的
主張正是「別拿別人的結論套自己」，在裡面放一份別人的作業等於自打嘴巴。
price in 的誤判恰恰都發生在「套到自己身上」那一步——那正是寫死範例
跳過的地方。

```
寫「教學／範例／說明」類元件前，先搜同類元件並讀它的類別註解
（這個專案的註解都寫了「為什麼這樣做」）。
範例一律用使用者當下的輸入現算，不寫死。
算式與既有 calculator 共用，不另寫一份——數字對不起來時，
使用者會先懷疑主功能，不是說明。
```

修正做法：新增 `PriceIn::TutorialExample`（吃 `ScenarioForm`，與圖表共用
`EntryReturnCalculator`），locale 文案改成 `%{ticker}` 佔位符，並用三個
資料旗標（`ttm`／`band`／`anchor`）決定哪幾節算得出來。

### 教訓 I：文案代入用 gsub，不要用 `String#%`

教學文案裡有「±15%」這種**字面百分號**，交給 `format`／`String#%` 會被當成
格式指示字元，直接拋 `ArgumentError`。要用 `%` 就得把每個字面百分號寫成
`%%`——那是把陷阱留給下一個改文案的人。

```ruby
text.gsub(/%\{(\w+)\}/) { vars[$1.to_sym]&.to_s || $0 }
```

**找不到的 key 原樣留著 `%{foo}`，不要換成空字串。** 畫面上出現 `%{foo}`
一眼就知道漏了；換成空字串會變成一句讀起來通順、但意思錯掉的話——
最難發現的那種 bug。

補測試掃描「教學區塊內不得出現 `%{`」時要**限定範圍**：
`price_in.export.brand` 本來就帶著 `%{ticker}` 給前端匯出時代入，
整頁掃會被它誤判成 bug（我第一版就是這樣紅的）。

### 附帶抓到的真 bug（都不會報錯，只會安靜地錯）

| Bug | 症狀 |
|---|---|
| `compact_blank` 把 `""` 與 `false` 當空值 | 使用者填的空字串被靜靜換成預設值，驗證永不觸發 |
| 快取存 `Data` 物件 | 改欄位後反序列化 `TypeError` → 500，錯誤訊息與真因無關 |
| `Data#to_h` 是淺層的 | 巢狀欄位存進快取仍是 Data，讀回來全 nil |
| `if (!window.driver) return` | 按鈕點下去毫無反應，查了四層才找到 layout 的條件式 |
| 漏跑 `tailwindcss:build` | 新 class 沒編譯，白字畫在白底上 |
| 「只在欄位空著才填」的防護 | 使用者按「重抓」時什麼都不會變 |
| 區間兩端除錯邊（`price / low` 當低端） | 得到上下顛倒但兩端都「看起來合理」的區間，肉眼掃過去不會停 |

共同形狀：**防護與快取讓失敗變得安靜**。加防護時要問
「它擋掉的是錯誤，還是使用者的明確要求？」

最後一列是另一種形狀：**值域正確、語意顛倒**。隱含倍數 = 現價 ÷ EPS，
高 EPS 對應**低**倍數，所以區間低端必須用 `est.high` 去除。`18.2 - 24.8`
寫成 `24.8 - 18.2`，兩個數字都對，只是順序錯了。這類錯誤唯一的防線是
**測試明確寫出哪一端由哪個端點算出**，不能只驗「有值」。


## 2026-08-30 — 今天的主線：失敗不長得像失敗

一天之內遇到三件互不相關、但同一個形狀的事：

| 事件 | 表面上 | 實際上 |
|---|---|---|
| Brakeman | exit code 非 0 + 一行版本提示 | **掃描根本沒執行** |
| pm2 排程 | 每 30 分鐘準時觸發、log 有輸出 | **時區錯 8 小時，3.5 個月零資料** |
| WebMock 樁 | 719 個測試全綠 | **樁被 catch-all 蓋掉，完全沒在提供資料** |

三者都沒有 stack trace、沒有紅字、沒有錯誤訊息。**能被看見的失敗其實是好運**；
真正危險的是那些「看起來一切正常」的失敗。

### 教訓 A：pm2 的 cron_restart 走的是 daemon 時區

```
/etc/systemd/system/pm2-idarfan.service 沒設 TZ
  → daemon 沿用 /etc/localtime（Asia/Taipei）
  → cron "13-20" 被當成台北時間，而不是腳本註解假設的 UTC
```

**註解寫了假設不等於假設成立。** 那句「pm2 daemon 必須以 TZ=UTC 啟動」
在 repo 裡躺了幾個月，沒有任何東西在檢查它。

```
凡是「某某必須如何設定」的註解，要嘛讓它自動成立（drop-in、設定檔），
要嘛加一個會失敗的檢查。只寫在註解裡的假設遲早會不成立。
```

### 教訓 B：排程壞掉的唯一症狀是「沒有新資料」

排程不會拋例外、不會有 exit code、log 甚至看起來正常
（intraday 每次都乖乖印「非交易時段，跳過」——那是**正確的行為**，
錯的是它被叫醒的時間）。

唯一可靠的偵測是**主動比對資料新鮮度**：
```bash
bin/audit freshness   # rake ops:freshness
```
容許值要放寬（本專案用 3–5 個交易日）。要抓的是「壞了好幾週沒人發現」，
不是「今天這次沒跑到」——太敏感會變成沒人看的雜訊，那就跟沒有一樣。

### 教訓 C：WebMock 是「後註冊者優先」

```ruby
stub_request(:get, %r{finnhub\.io/api/v1/quote}).to_return(真實形狀)
stub_request(:get, %r{finnhub\.io/}).to_return(空物件)   # ← 蓋掉上面那條
```

catch-all 必須**先**註冊，具體端點後註冊才蓋得過它。寫反了的後果特別陰險：
所有請求都拿到空物件，服務優雅降級，頁面照樣渲染，**719 個測試全綠**。

反向驗證過：把 catch-all 移回最後時，`spec/config/external_apis_spec.rb`
紅 2 項，而 **223 支 request spec 全部照樣通過**。

```
測試基礎設施本身也需要測試——它壞掉時不會有任何測試變紅。
```

### 教訓 D：`Rails.cache` 在 test 環境是 :null_store

服務層看起來有快取保護（`Rails.cache.fetch`），實際上在測試裡**每次都出網**。
讀 `config/environments/test.rb` 才會發現 `cache_store = :null_store`。

判斷「這段程式碼在測試裡會不會打外網」時，不能只看有沒有包 cache——
要看那個環境的 cache_store 是什麼。

### 教訓 E：驗證稽核工具「真的掃了東西」，不能只看 exit code

```
exit 0 或非 0 + 輸出只有一兩行 = 高度可疑，先當成沒跑
要找的是規模證據：掃了幾個檔、幾個 controller、跑了哪些 check
```


## 2026-08-30 — 待辦清單自己也會過期

### 教訓 A：動手前先查證這件事是不是已經做完了

我把「`tracked_tickers` 沒有 per-user ownership」列在待辦，還估算了「要重新
key 81 萬筆 `option_snapshots`」的風險。實際打開檔案才發現**設計決定早就做完、
把關已經存在、註解寫得清清楚楚**，而且 model 與 controller 兩處都有記載。

差一點就為了一個不存在的問題去動 81 萬列資料。

**防治規則：**
```
待辦項目在動手前一律重新查證：
  * 先讀相關 controller / model 的檔頭註解——設計決定通常寫在那裡
  * 查 git log 看該檔案最近有沒有相關改動
  * 跑一次相關 spec 看行為是不是已經被釘住
自己列的待辦不等於現況，尤其是跨了好幾輪工作之後。
```

### 教訓 B：`before_action ... only: [...]` 的每個 action 都要有測試

```ruby
before_action :require_admin!, only: %i[create update destroy collect]
```
只測 `destroy` 時，把 `create` 從清單裡拿掉不會有任何測試失敗。
`only:` 清單有幾個 action，就要有幾個「非授權者被擋下」的測試。

這和 CompositeSignalService 的門檻是同一類問題：**測了「有作用的那一格」，
沒測「相鄰的那一格」，改動就看不出來。**

### 教訓 C：權限 UI 缺席時，使用者看到的是「功能壞掉」

後端有 `require_admin!` 擋住，但前端照樣顯示按鈕，按下去回一句「新增失敗」。
安全上沒問題，**體驗上是把權限限制偽裝成 bug**。

加權限旗標時要在註解寫明「這不是安全邊界」，免得後人以為拿掉旗標
就等於開放權限——真正的把關永遠在後端。
