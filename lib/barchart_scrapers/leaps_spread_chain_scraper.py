"""
LEAPS 垂直價差 — Stage 2：指定到期日的 Call 鏈抓取（leaps-call-spread-spec P1）。

2026-09-25 自 bcvs_call_chain_scraper.py 逐字複製（使用者裁示：垂直價差不碰 bcvs，
sidecar 各自一份），只改這段說明。導覽指定 expiration 的 options 頁
（view=sbs&moneyness=100，全部履約價），解析 Call 側每個 strike 的 Barchart 欄位，
缺欄位回 null、不造值，也不做任何篩選（bid、ask 皆 0 的列要留給「盤後參考價」）。
讀的是頁面 bc-data-grid 元件的資料（_data[].raw），不攔截網路請求。直連 CDP 9222。

Barchart 改版時，bcvs_call_chain_scraper.py 與本檔都要檢查。

Usage:  python3 leaps_spread_chain_scraper.py SYMBOL EXPIRATION

EXPIRATION 為 Barchart 原始 value（例如 "2027-10-15-m"，來自
leaps_spread_expirations_scraper.py 的輸出）。
