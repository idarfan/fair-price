...
沿用 pmcc_short_call_scraper.py 的 OPTIONS_PRICES_JS 為底，改篩 Put 側
（原本篩 Call），直連 CDP 9222。

Usage:  python3 bpus_put_chain_scraper.py SYMBOL EXPIRATION

EXPIRATION 為 Barchart 原始 value（例如 "2026-08-21-m"，來自
bpus_expirations_scraper.py 的輸出）。
...
# Stage 2: 指定 expiration 的 Put 側全履約價。實測(2026-07-13, RKLB)發現：
# 不帶 view=sbs 時頁面只掛一個 bc-data-grid（Call 側，跟 pmcc_short_call_scraper.py
# 用途一致）；帶 view=sbs 後頁面會同時掛 3 個 bc-data-grid（Call / Put / 第三個
# type 為 null，疑似合併用途），Put 資料在其中一個，順序不保證——因此不能只抓
# document.querySelector 的第一個，要掃全部 grid 再用 optionType 篩選 Put。
PUT_CHAIN_JS = """
(() => {
  ...
  return rows.filter(r=>r.optionType==='Put'||r.symbolType==='Put')
    .map(r=>({
...
async def main(symbol, expiration):
    ...
    rows = await _wait_for_grid(ws_url, PUT_CHAIN_JS, max_wait_s=GRID_MAX_WAIT_S)
    ...
    if not rows:
        confirmed = await _confirm_empty(ws_url, PUT_CHAIN_JS, target_id=target_id)
        if confirmed:
            rows = confirmed
    ...
    # 現價常比 grid row 資料晚一拍渲染完（實測見 bpus_expirations_scraper.py），
    # 但這裡 rows 已確認非空，代表 grid 早已 mount，直接讀一次通常就夠；仍保留
    # 一次重試以防同一批 digest cycle 尚未更新 moneyness 反推用的欄位。
