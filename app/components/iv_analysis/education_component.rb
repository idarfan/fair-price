...
Wheel: 賣 Put（CSP）或賣 Call（CC）",
  ...
  # JavaScript 已搬到 app/frontend/behaviors/ivEducationChainTooltip.js（稽核 H-3）。
  # entrypoints/behaviors.ts 會依 data-behavior 動態載入該模組。
  def render_chain_tooltip_script
    div(data: { behavior: "iv-education-chain-tooltip" })
  end


  # JavaScript 已搬到 app/frontend/behaviors/ivEducationChart.js（稽核 H-3）。
  # entrypoints/behaviors.ts 會依 data-behavior 動態載入該模組。
  def render_chart_script
    div(data: { behavior: "iv-education-chart" })
  end
  # JavaScript 已搬到 app/frontend/behaviors/ivEducationTts.js（稽核 H-3）。
  # entrypoints/behaviors.ts 會依 data-behavior 動態載入該模組。
  def render_tts_script
    div(data: { behavior: "iv-education-tts" })
  end
end
