...
class TechnicalDashboard::PageComponent < ApplicationComponent
  ...
  def render_options_charts
    ...
    # JavaScript 已搬到 app/frontend/behaviors/techDashOptionsCharts.js（稽核 H-3 Wave 2）。
    # 原本的 Ruby 插值改成 data attribute 傳入。
    div(data: { behavior: "tech-dash-options-charts", symbol: symbol })
  end


  def render_dte_filter_script
    # JavaScript 已搬到 app/frontend/behaviors/techDashDteFilter.js（稽核 H-3 Wave 2）。
    # 原本的 Ruby 插值改成 data attribute 傳入。
    div(data: { behavior: "tech-dash-dte-filter" })
  end

  def render_loading_script
    csrf = helpers.form_authenticity_token rescue ""
    # JavaScript 已搬到 app/frontend/behaviors/techDashLoading.js（稽核 H-3 Wave 2）。
    # 原本的 Ruby 插值改成 data attribute 傳入。
    div(data: { behavior: "tech-dash-loading", csrf: csrf })
  end
  # ---------------------------------------------------------------------------
  ...
  def render_max_pain_filter_controls(symbol, mp)
    ...
    render_max_pain_filter(symbol)
  end

  def render_max_pain_filter(sym)
    # JavaScript 已搬到 app/frontend/behaviors/techDashMaxPainFilter.js（稽核 H-3 Wave 2）。
    # 原本的 Ruby 插值改成 data attribute 傳入。
    div(data: { behavior: "tech-dash-max-pain-filter", symbol: sym })
  end
end
