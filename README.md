...
他是對的，而且這個錯誤在同一個專案裡有現成的反例——`LogicExampleBuilder`
早就是「拿你目前填的數字跑一次」。在一個以「別拿別人的結論套自己」為主張的
工具裡放一份別人的作業，等於自打嘴巴：price in 的誤判恰恰都發生在
「套到自己身上」那一步，而那一步正是寫死範例跳過的地方。

**`PriceIn::TutorialExample`**（新增）：吃 `ScenarioForm`，吐出代入值與兩張表。
算式與圖表共用 `EntryReturnCalculator`，不另寫一份——教學裡的數字若跟圖上的
對不起來，使用者第一個懷疑的會是圖。

locale 文案改成 `%{ticker}`／`%{price}` 這類佔位符。**代入用 gsub 不用
`String#%`**：文案裡有「±15%」這種字面百分號，交給 `format` 會被當成格式
指示字元直接拋 `ArgumentError`。找不到的 key 原樣留著 `%{foo}` 而不是換成
空字串——畫面上看到佔位符一眼就知道漏了，換成空字串會變成一句讀起來通順
但意思錯掉的話（已補測試掃描教學區塊內不得出現 `%{`）。

**三個資料旗標決定哪幾節顯示得出來**：`ttm`（按過帶入現價）、`band`（填了
預測區間）、`anchor`（有未來 EPS 的錨）。缺的那節顯示「補什麼就會算給你看」，
不顯示半套數字——半套數字比沒有更容易誤導。第五節的「倍數與盈利不可能並存」
那句還額外需要 TTM EPS，因此 `extra` 與整節的 `requires` 分開判定。

實測（NOK、自填倍數 12/20/28、買入價 \$8.50）：

```
NOK 現價 $10.92 ÷ TTM EPS $0.13 = 86.2 倍
NOK：$10.92 ÷ $0.60 = 18.2 倍，$10.92 ÷ $0.44 = 24.8 倍
12 倍 → $6.00  −45.1%      $10.92（買入基準） → −8.4%
20 倍 → $10.00  −8.4%      $8.50              → +17.6%
28 倍 → $14.00 +28.2%
```

涉及檔案：`app/services/price_in/tutorial_example.rb`、
`app/components/price_in/tutorial_component.rb`、
`config/locales/price_in.zh-TW.yml`、`spec/requests/price_in_spec.rb`

---

### 2026-09-09（三）— 新增：Price-In 隱含倍數對照列、頁底工具教學

**隱含倍數（現價 ÷ 分析師預測 EPS）**，加在「估值對照」的第四列。
這是判斷貴賤唯一不需要使用者填任何假設的入口：股價與分析師預測都是抓回來的。

存在的理由是同一天實測 NOK 時發現的盲點：`本益比 (P/E)` 用 TTM EPS 當分母，
`10.93 / 0.1267 = 86.27` 倍。這個數字讀起來像「市場願意給 86 倍」，實際上只是
盈利被壓縮、分母太小的算術結果。換成 FY2027 預測 EPS 當分母，`10.93 / 0.60` 到
`10.93 / 0.44` = **18.2 – 24.8 倍**——後者才是市場真正的定價基準。

三個實作決定：

1. **優先取下一財政年度**，取不到才退回本財政年度，年度標籤跟著換。
   本財政年度通常已過大半、剩餘變數少，對「現在買貴不貴」的解釋力低。
2. **刻意不給「帶入」按鈕**（理由同產業平均）：填進圖 A 的倍數，反推出來的
   所需 EPS 必然等於分析師預測本身，又是一次循環論證。它是對照，不是假設。
3. **伺服器端與前端各算一次且必須同義**：JS 負責「按下帶入現價立即更新」，
   Ruby 負責「重新出圖整頁重載之後仍然在」。少任何一邊，這個數字都會在
   某個操作之後憑空消失——那正是 09-09 上午「估值對照被清空」那個 bug 的形狀。

容易寫反且不會被發現的地方：高 EPS 對應**低**倍數，區間低端必須用 `est.high`
去除。寫反會得到上下顛倒但兩端都「看起來合理」的區間，已補測試釘住。

**Price-In 工具教學**（`PriceIn::TutorialComponent`），頁面最底、預設收摺。
與既有兩種說明卡的分工：欄位說明卡答「這一格要填什麼」、判讀說明卡答「這張圖
怎麼看」、本元件答「這個工具在回答什麼問題，以及最容易誤讀的地方」——後者
不屬於任何單一欄位或單一圖，原本無處可放。

九節，全部用 NOK 的真實數字貫穿（抽象規則看得懂不代表套得到自己的情境），
其中三節是當天實際踩到的誤讀：圓點落在色帶左側不等於便宜、倍數若來自現價
反推則圖 A 是恆等式、報酬的權重倍數不低於盈利（固定 EPS $0.50 只改倍數，
15 倍是 −31.4%、80 倍是 +266.0%）。參數錯誤而不出圖時教學照樣顯示——
那正是使用者最需要它的時候。

文案全部進 locale，元件不硬編句子；表格存成 `headers` ＋ `rows` 結構化資料，
排版留在 view。報酬欄依正負走既有的 `change_color`（綠漲紅跌），不另立一套配色。

涉及檔案：`config/locales/price_in.zh-TW.yml`、
`app/components/price_in/tutorial_component.rb`、
`app/components/price_in/{page,input_form}_component.rb`、
`app/frontend/behaviors/priceInTicker.ts`、`spec/requests/price_in_spec.rb`
