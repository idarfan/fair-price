...
RSpec.describe BpusFetchExpirationsJob, type: :job do
  let(:symbol) { "RKLB" }
  ...
  describe "#perform — success path" do
    before do
      svc = instance_double(BarchartScraperService, fetch_bpus_expirations: { status: "success", errors: [] })
  ...
  describe "#perform — barchart_session_expired path" do
    before do
      svc = instance_double(BarchartScraperService, fetch_bpus_expirations: { status: "barchart_session_expired" })
      allow(BarchartScraperService).to receive(:new).with(symbol).and_return(svc)
    end
  ...
  describe "#perform — no_candidates path" do
    before do
      svc = instance_double(BarchartScraperService, fetch_bpus_expirations: { status: "no_candidates" })
      allow(BarchartScraperService).to receive(:new).with(symbol).and_return(svc)
    end
  ...
  describe "#perform — error path" do
    before do
      svc = instance_double(BarchartScraperService, fetch_bpus_expirations: { status: "error", errors: [ "CDP unavailable" ] })
      allow(BarchartScraperService).to receive(:new).with(symbol).and_return(svc)
    end

    it "maps to error status with errors" do
      expect(Rails.cache).to receive(:write).with(
        "bpus_job_#{job_id}",
        { status: "error", errors: [ "CDP unavailable" ] },
        expires_in: 5.minutes
      )
      described_class.perform_now(symbol, job_id)
    end
