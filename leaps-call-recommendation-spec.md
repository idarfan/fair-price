...
- **階段 C.5（新增，Phase F 開始前要先完成）**：修復上面標出的 V&G session 過期設計缺口
  - V&G 抓取那一層偵測到 session 過期時，要往上回報成 `partial_error`，跟 Options Prices 那層用同一套機制，不能讓它降級成 null 欄位後被當成正常結果。
  - `result[:errors]` 要明確寫出是「V&G 在抓哪個到期日時斷線」，跟 Options Prices 斷線的訊息要能區分開來（使用者/前端要看得出是哪一層斷的，不是只知道「有斷線」）。
  - 對應第8節「錯誤訊息分情況顯示」那條：前端目前分的三種情況（未登入／partial_error／其他例外）裡，`partial_error` 這一類現在會出現兩種子情況（Options Prices 斷線 vs V&G 斷線），訊息文字要能反映是哪一層斷的，不能兩種都顯示同一句「Session 過期」。
  - 單元測試：模擬 V&G 單獨斷線（Options Prices 正常）的情境，驗證 `result[:status]` 確實是 `partial_error`，不是 `success`。
- **階段 C.5b（新增）**：Controller CDP 連線預檢——已開始實作（在 `leaps_recommendations_controller.rb` 加預檢），需確認：(1) 預檢失敗時回報時間確實在 1-2 秒內，不是等到 scraper 內部才報錯；(2) 錯誤訊息符合第8節表格裡新增的那一類措辭；(3) 補測試覆蓋「CDP 離線時 controller 直接擋下、不送 job」。
...
- **階段 E**：⚠️ 三項已逐一確認，**兩項通過、一項未通過**：
  ...
  - ❌ **配色未通過，需補**：`LeapsRecommendationsController` 對應 page_component 自己定義了 `LIQUIDITY_STYLE`／`DIR_STYLE` 兩個常數，跟三維度儀表板既有的 `DIV_META`（`warning`/`caution`/`confirm_bull`/`confirm_bear`）**顏色數值相同但是各自獨立定義的兩份，不是共用**——這是巧合對齊，不是規格要求的「直接複用同一組顏色變數/CSS class」。待辦：把 `LIQUIDITY_STYLE`/`DIR_STYLE` 搬到共用位置（例如 `ApplicationComponent` 或獨立 concern），LEAPS 跟三維度儀表板兩邊 `include` 同一份定義，不要各自維護。
...
- [ ] **（已修正方向）** 排行表格與推薦分析都套用 `DTE>=364`（52週）硬性下限——這是這次新加的修正，不是「不設天數門檻」；表格不會再出現 5、13、20 天這種明顯不是 LEAPS 的近期合約。排序仍只用 OI（主）+ DTE（次），沒有把 Options Flow 數字混進排序。
...
- [ ] **（新增）** 頁面最上方有獨立的「推薦分析」區塊，近天期／遠天期各一組明確建議＋完整理由文字（到期日、履約價、Delta、跟次選的 OI/流動性對比、Time Value%、Spread%、Vega/IV 佐證），不是又一張表格；下半段完整排行表維持表格形式，不寫每列理由。
- [ ] **（新增）** 某個天期區間沒有任何符合條件的候選時，畫面明確顯示「此天期區間目前沒有符合條件的候選」，不是留白或整段消失。
- [ ] **（新增）** 若某組推薦本身帶有「近期無成交」警示（因為該組全部候選都有警示），理由文字裡有明確點出這個警示，沒有因為它是「推薦」就把警示蓋掉不提。
...
- [ ] **（更新）** V&G session 過期已正確傳播為 `partial_error`：模擬 V&G 單獨斷線（Options Prices 正常）的情境，`result[:status]` 確實變成 `partial_error`，不會安靜降級成 Vega/itm_probability 為 null 但整體狀態顯示成功；`result[:errors]` 訊息能區分是 Options Prices 斷線還是 V&G 斷線。
- [ ] **（新增）** 抓取改用「鎖履約價、Stacked 檢視」兩階段策略：第一階段用 Near the Money 檢視的 Delta 欄位（不是價格反推公式）篩 `Delta>=0.80` 估候選履約價、第二階段針對這些履約價（含上下緩衝）各拿全到期日資料，取代逐到期日序列爬；同一 symbol 的總頁面導航次數有實測下降，且最終候選清單跟舊版逐到期日序列爬的結果一致（沒有因為換抓取方式漏掉本該存在的候選）。
- [ ] **（新增）** `0.80` 跟 `0.75–0.90` 是兩條不同用途的規則，沒有被誤合併成同一個門檻：Stage 1 用 `Delta>=0.80`（無上限）挑要鎖定的履約價，Stage 2 拿到完整資料後仍套用 `0.75–0.90` 區間做最終篩選——有測試覆蓋「Stage 1 選中的履約價，在某個到期日 Delta 實際落在 0.80–0.90 之間」跟「同一履約價在另一個到期日 Delta 超過 0.90 或低於 0.75」這兩種情況，驗證 Stage 2 篩選確實會把後者排除，不會因為 Stage 1 篩過就直接收進最終候選。
- [ ] **（待修）** 奇數列底色實際 class 是 `bg-gray-50/50`（含透明度），目前回報的是 `bg-gray-50`（缺 `/50`），需要補上透明度後再驗收一次畫面。
- [ ] 同一 symbol 5 分鐘內重複查詢會讀快取，不重複打 Barchart。
- [ ] 表格下方有「僅供策略篩選參考，非投資建議」提示文字。
- [ ] **（新增）** Controller 在送出抓取 job 前先做 CDP 連線預檢，連不上時在 1-2 秒內回報明確訊息（提示檢查 Windows Chrome 的 remote-debugging port），不是等 job 跑到 scraper 內部才在 13 秒後報錯；有測試覆蓋「CDP 離線時 controller 直接擋下不送 job」這個情境。
- [ ] 抓取失敗時的畫面訊息依 `result[:status]` 分情況顯示（未登入／CDP預檢失敗／partial_error 帶出實際斷點到期日與斷線層級／其他例外），不是共用同一句固定字串；驗收時實際觸發過這四種情況各看一次畫面。
- [ ] **（已查明未通過，待補）** 配色（卡片底色、邊框、綠/橘黃/紅語義色、hover 效果）直接讀取並複用三維度儀表板既有的 CSS/變數，沒有另外設計一套新色票；流動性分級與 Options Flow 看多/看空判斷的顏色語義跟 `divergence_flag` 的 confirm/warning/caution 對應一致。**現況：LEAPS 的 `LIQUIDITY_STYLE`/`DIR_STYLE` 跟三維度儀表板的 `DIV_META` 是各自獨立定義、數值巧合相同的兩份常數，不是共用同一份——待補：搬到共用位置（`ApplicationComponent` 或獨立 concern），兩邊 `include` 同一份，不要各自維護。**
