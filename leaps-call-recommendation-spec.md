...
## ✅ 接手前必讀：功能已結案（2026-07-02）。⚙️ **2026-07-03 補充修復**：`leaps_scraper.py` 抓取誤判 bug（固定 sleep 導致 `bc-data-grid._data` 未載入即誤判 session 過期）已修復，見第3節與第4節補充記錄。🆕 **2026-07-04 新增 Phase H（進行中）**：`leaps_option_chain_snapshots` 加入 `intrinsic_value`／`extrinsic_value` 兩個衍生欄位（以 Mid 計算），排行表新增外在價值/外在佔比顯示欄，見「Phase H」一節。🆕 **2026-07-04 新增 Phase I（未開始）**：LEAPS 頁面右上角加「匯出 PNG／匯出 PDF」按鈕（純前端 client-side，整頁完整匯出），見「Phase I」一節。原結案範圍不受影響；Phase H/I 是使用者明確決定附加在本規格的小型增量（跟本功能頁面緊密耦合，不值得另開文件），不是推翻「新功能另開規格」的原則。
...
- **階段 H（🆕 2026-07-04 新增，未開始）**：`intrinsic_value`／`extrinsic_value` 衍生欄位 → 確認
  ...
  **測試要求**：
  ...
  5. **人工已知值對照**：用使用者 2026-07-03 提供的 NVTS 截圖數值抽驗至少兩筆，例如 2028-01-21 鏈、現價 14.46：strike 5（Mid 11.00 → 內在 9.46、外在 1.54、佔比 14%）、strike 10（Mid 9.33 → 內在 4.46、外在 4.87、佔比 52%）。程式算出的值必須跟人工驗證值一致（容許四捨五入位數差異），對不上就是公式或欄位接錯，不是「差不多就好」。

- **階段 I（🆕 2026-07-04 新增，未開始）**：頁面匯出 PNG／PDF → 確認
  ...
  **架構決定（實作前先讀，不得擅自改走後端方案）**：
  ...
  2. 函式庫用 CDN 載入（對齊 Tailwind CDN 的既有模式）：PNG 用 `html2canvas`（或 `html-to-image`，實作者擇一後在交付說明裡寫明選擇理由），選定後**兩個匯出共用同一個渲染路徑**。PDF 用 `jsPDF`。
...
### 🆕 fresh window 5 → 30 分鐘（2026-07-04 新增，全部未驗收；⚠️ 已有多次「口頭回報已改但實際仍是 5 分鐘」的前科，不附證據不算完成）

- [ ] 建立單一常數（如 `LeapsOptionChainSnapshot::FRESH_WINDOW = 30.minutes`），model `fresh` scope、`fetch_leaps`、`fresh_data_exists?`、相關 `Rails.cache` `expires_in` 全部引用它。
- [ ] `grep -rn "5\.minutes\|expires_in.*300" app/ lib/` 無殘留舊值，grep 輸出附在回報裡。
- [ ] 邊界測試（`travel_to`）：`scraped_at` 29 分鐘前 → cache hit 不排 job；31 分鐘前 → cache miss 重新抓取。測試用 `FRESH_WINDOW ± 1.minute` 表達，不寫死 29/31。
- [ ] 核心情境實測：同一 symbol 在**間隔 >5 分鐘、<30 分鐘**時重查，Rails log 證明走 cache 沒觸發 scraper（這個區間才能鑑別新舊行為；5 分鐘內重查驗不出差異，不算通過）。
...
### 🆕 Phase H（2026-07-04 新增，全部未驗收）

- [ ] Migration 只加 `intrinsic_value`、`extrinsic_value` 兩欄（decimal，precision/scale 對齊既有報價欄，允許 null），「外在佔比」沒有落地成欄位。
- [ ] 兩欄的計算發生在 Ruby `persist_leaps` 層，`leaps_scraper.py` 零改動（`git diff` 可證）。
- [ ] 權利金基準是 `mid = (bid + ask) / 2`，**不是 `last_price`**——code review 時明確確認公式裡沒有出現 `last_price`。
- [ ] 公式依 `option_type` 分支（call：`max(0, spot - strike)`；put：`max(0, strike - spot)`），put 分支有單元測試釘住。
- [ ] bid/ask 任一為 null 時兩欄存 null（不是 0）；display 層 mid = 0 或欄位 null 時外在佔比顯示「—」（不是 0% 或 NaN）。
- [ ] 既有舊 rows 已 backfill（同一公式），排行表不會出現部分列有值、部分列空白的混雜畫面。
- [ ] `time_value_pct` 改讀已存 `extrinsic_value`，排行層沒有第二份內在/外在價值計算式（全 codebase 只有 persist 層一處公式）。
- [ ] 排行表顯示「內在價值／外在價值／外在佔比」三欄，外在佔比與 Time Value% 兩欄同時存在、分母不同（前者 mid、後者 underlying_price），沒有被合併成一欄。
- [ ] request spec 覆蓋完整 HTTP 路徑，驗證回應包含新欄位且數值正確（不是只有 service 單元測試）。
- [ ] **核心情境 E2E**：不帶 user_strike 的最基本查詢實際跑過一次，新欄位全數有值；只測過帶 user_strike 的情境不算通過此項。
- [ ] **人工已知值對照通過**：NVTS 2028-01-21 鏈（現價 14.46）strike 5 → 內在 9.46/外在 1.54/佔比 14%；strike 10 → 內在 4.46/外在 4.87/佔比 52%。程式輸出與人工值一致。

### 🆕 Phase I：頁面匯出 PNG／PDF（2026-07-04 新增，全部未驗收）

- [ ] 「匯出 PNG」「匯出 PDF」兩顆按鈕位於頁面右上角，沿用深色主題樣式，事件綁定用事件委派（`git diff` 無 inline onclick）。
- [ ] 純前端實作，零後端變動：`git diff` 中沒有 routes/controller/service 的修改。
- [ ] PDF 是由 PNG 圖片嵌入產生（非 jsPDF 文字 API），PNG 與 PDF 畫面一致。
- [ ] 匯出內容為完整頁面（不只視窗可見範圍），按鈕本身與導覽列不出現在輸出裡。
- [ ] 檔名符合 `leaps_{symbol}_{YYYYMMDD_HHmm}` 格式；無資料時按鈕 disabled；匯出中防重複點擊。
- [ ] **E2E 開檔驗收**：實際下載 PNG 與 PDF 各一份並打開檢查——排行表完整未腰斬、深色背景正確、中文無亂碼——回報附上輸出檔或其截圖。「有下載」不等於通過。
