...
- **階段 C**：✅ 已完成，但**有兩項待補，不能算全部結束**：
  1. `LeapsRankingService` 當時實作沒有 `DTE>=364` 門檻（這個門檻是這次新加的修正，Phase C 完成時規格還沒寫這條），需要追加：`fetch_candidates` 篩選邏輯要在 Delta 篩選之前/之後加上 `dte >= 364`，25個既有測試應該不受影響（門檻是新加的範圍限制，不是改變既有邏輯），但要補新測試驗證 `dte<364` 的候選確實被排除。
  2. ⚠️ **V&G merge bug 尚未確認修復**：實測 NOK 時發現 4 個候選裡有 3 個的 Vega／被指派機率顯示「—」，懷疑是 `_merge_vg` 的 key 比對（strike/expiration_date 型態或格式不一致）有問題，這個在規格裡已經要求 Claude Code 提供 `_merge_vg` 實際邏輯＋兩邊頁面的 key 清單比對，但截至目前還沒有收到修復回報，**Phase F 開始前最好先確認這個 bug 的狀態**，因為推薦分析層的理由文字會引用 Vega／被指派機率，如果底層 merge 還是壞的，推薦理由會帶著缺值上場。
...
- [ ] **（新增，未確認）** V&G merge bug 已修復：實測同一個 symbol 查詢，候選裡不會再出現「OI/Delta 有值但 Vega/被指派機率顯示 — 」的不一致情況（除非該候選本身在 V&G 頁面真的查無資料，且這種情況要能跟 merge 失敗區分開來）。
