...
（不繼續往下）                  （使用者填了履約價 → 直接當中心點；
                                ...
                                存進 PostgreSQL
                                       ↓
                                跑 LEAPS 候選排行 + 推薦分析（第 6 節，套用 Delta >= 0.60 最終篩選）
                                       ↓
                                畫面顯示：
...
1. **第一階段：估出候選履約價**——選擇 Barchart「Near the Money」檢視（畫面上預設靠近現價的履約價清單），這個檢視本身就直接顯示 Delta 欄位，**不需要另外用股價/權利金反推公式去算錨點**（**禁止**用股價/權利金反推公式估算錨點——已定案放棄，論證見 history 第 4 節）。直接讀 Delta 欄位，篩出 **Delta >= 0.60** 的履約價，當作第二階段要鎖定的候選對象。
   - **這個 0.60 門檻用在兩個地方，但作用對象不同，不是同一個判斷**：Stage 1 用 Delta>=0.60（無上限）在 Near the Money 檢視上快速圈出值得鎖定深入查詢的**履約價**；Stage 2 拿到這些履約價在所有到期日的完整資料後，**每一筆（履約價＋到期日）都還要再套用 Delta>=0.60 這個下限做最終篩選**（因為同一履約價在不同到期日的 Delta 會漂移，Stage 1 篩過的履約價，到了某些到期日 Delta 可能落到 0.60 以下——這要交給 Stage 2 的逐筆篩選去判斷，不是 Stage 1 篩過就直接算數）。**2026-08-05 起兩階段都不設上限**：深度價內（Delta 逼近 1）合約一樣會被列出，讓使用者自行判斷是否要付出接近正股的成本換取更低槓桿。
   - **新增：使用者可以手動輸入履約價，覆寫 Stage 1 的自動估算**——輸入框是選填，留空時維持現有的 Delta>=0.60 自動偵測；填了之後，**這個輸入值直接取代 Stage 1 算出來的「中心履約價」，後面的流程完全不變**：仍然以這個值為中心，照第4點的規則上下加緩衝檔，再進 Stage 2 鎖履約價拿全到期日資料，最終一樣套用 Delta>=0.60 篩選。這代表手動輸入**不是「只查這一檔」、也不是「跳過 Delta>=0.60 篩選」**，只是換掉 Stage 1 那一步的資料來源（從自動偵測換成使用者指定），Stage 2 跟最終篩選邏輯一律不變。
   - **邊界情況**：如果使用者輸入的履約價跟現價差太遠（例如根本不是深價內，或反而是價外），Stage 2 抓回來的資料在套用 Delta>=0.60 篩選後可能完全沒有候選通過——這種情況要明確顯示「這個履約價（含緩衝檔）在所有到期日都沒有符合 Delta>=0.60 的候選」，不要顯示空白或誤導成查詢失敗，這是輸入值本身不適合做 LEAPS、不是系統錯誤。
2. **第二階段：逐履約價拿全到期日資料**——針對第一階段選出的每個履約價，各開一次「Stacked + 鎖履約價」頁面，一次拿到該履約價在所有到期日的數值。原本「N個到期日 × 2頁」的序列爬蟲，縮減成「2–4個履約價 × 2頁」。
3. **篩選邏輯不變**：拿到資料後，還是套用 Delta>=0.60 篩選——**這一步不能省略**，鎖履約價只是換一種方式收資料，不代表這幾個履約價在每個到期日都符合 Delta 範圍，仍要逐筆檢查。
...
- **下半段：完整排行表**（原有設計，本節調整：加上 52 週門檻）——所有符合 `DTE>=364` 且 Delta>=0.60 的候選都列出來，OI 高到低排序，使用者可以看到推薦之外的其他選項。
...
- Delta 仍用下限篩選縮小候選範圍到「深度價內」，預設門檻維持 Delta>=0.60（無上限，2026-08-05 移除 0.90 上限），可調。
...
| `delta_min` | 0.60（無上限，2026-08-05 移除 `delta_max` 0.90） | 深度價內目標 Delta 下限（用來定義「這是不是 LEAPS Call 候選」，不是流動性篩選），可調 |
...
- **要加進現有的導覽列/選單**：找到目前 Phlex layout 裡放導覽連結的位置（例如 header 或 sidebar 的 nav 元件），把這個新頁面的連結加進去，不能只新增 controller/route 卻沒有任何入口可以點進去，使用者不該需要自己手動輸入網址才能用到這個功能。
  - **現行位置（已交付事實）**：`app/components/fair_value/app_switcher_component.rb` 的 `APP_LINKS` 陣列（第 12 項）：`{ icon: "📐", label: "LEAPS 排行", href: "/leaps", desc: "深度價內 Call 候選排行 · Delta ≥ 0.60 · OI 排序" }`（2026-08-05 隨 `delta_max` 上限移除同步更新）。所屬應用程式：FairPrice（同一 Rails process，port 3003，非獨立 app）。
- Phlex 元件，沿用專案既有慣例（不用 ERB/Hotwire）
- 輸入區塊新增**選填**的履約價輸入框（放在股票代號旁邊或下方），留空時走 Stage 1 自動偵測（Delta>=0.60），填了則直接當 Stage 2 的中心履約價，後續流程（上下加緩衝檔、Stage 2 抓取、最終 Delta>=0.60 篩選）完全不變，不是另開一套邏輯。
- 頁面流程：輸入股票代號（＋選填履約價）→ 送出後顯示抓取中狀態（Playwright 抓取需要數秒，用 ActiveJob + Turbo Stream/polling）→ 完成後顯示：
  ...
  2. **完整排行表格**（第 6 節下半段，OI 高到低排序，`DTE>=364` 且 Delta>=0.60 的候選都列出來）
