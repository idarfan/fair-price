...
| 14 | 匯出按鈕（S8 完成後才存在）| 帶著數字去吵架 | 下次有人說「已經 price in 太多」，把這張圖丟出去，問他：用哪一年的 EPS？什麼口徑？給幾倍？有沒有折現？四項答不出來的，就是在講廢話。 |
...
2. 禁止硬編機構名或目標價來源
3. ~~禁止 CDN 熱連~~ → **2026-09-07 變更**：Chart.js 與 annotation plugin 沿用 layout 既有的
   CDN 載入（見 §2 決議）。driver.js、html-to-image、jsPDF 仍為 vendor 本地檔
4. 禁止 Service 混入 view 依賴
5. 禁止用 U+2212 作負號
6. 禁止在 `fiscal_year_label` 缺席時產出圖表
7. 禁止為了單頁不捲動而縮字級或改兩欄擠壓。
   ~~禁止任何區塊鋪滿全寬（輸入卡 760px、圖表卡 960px）~~ → **2026-09-07 變更**：
   依使用者要求改為全版面 96%（見 §S4 容器寬度）。
   字級三級制仍適用於內文與圖表；輔助文字（欄位標籤、註腳、來源說明）為 16px
...
13. 禁止在品牌字串中硬編股票代號
14. 禁止把任何參考倍數**自動**填入倍數輸入框；禁止在參考值為 nil 時報錯或阻擋出圖。
    **2026-09-08 補充**：依使用者要求，本益比與預估本益比各配一顆「帶入」按鈕——
    那是使用者明確按下的動作，不是自動填入，兩者不衝突。產業平均本益比刻意不給
    按鈕：它是別人的倍數，不是這檔股票現在的倍數
