...
## 補做三（2026-07-08 使用者回報）：語意色未套用，推薦徽章完全遺漏

### 問題

使用者指出兩點：(1) Call/Put 總額在 HTML 是綠/紅兩色，PDF 卻是黑字；(2) 推薦分析卡片頂部的彩色徽章（`render_pick_badge`，$12.00/2027-12-17 綠色 pill、$10.00/2028-12-15 黃色 pill）完全沒有出現在 PDF——這是第三個被遺漏的區塊。

### 修復

- `PDF_SIGNAL_HEX` 擴充 `border`／`dot` 兩色（原本只有 `bg`／`text`），對照 HTML `SIGNAL_COLORS` 的 `bg-*-50`／`border-*-300`／`text-*-800`／`dot bg-*-400` 四色語意，四色皆有明確 hex 對應，不另造色票。
- `pdf_reco_group` 加 `badge: {text:, delta_text:, color:}`（`color` 複用 `pdf_signal_rgb_for_tier`，跟排行表流動性欄同一套語意判斷邏輯）。
- `flow_summary` 加 `call_color: "#16a34a"`（`text-green-600`）／`put_color: "#ef4444"`（`text-red-500`），跟 HTML `render_flow_panel` 的 class 一致。
- JS `renderRecoGroup` 加畫彩色徽章：`roundedRect` 畫圓角背景＋邊框、`circle` 畫圓點、彩色文字——模擬 HTML pill 外觀。
- JS `renderFlowTable` 的總額改成分段畫（Call 綠字＋分隔符灰字＋Put 紅字），不再合併成一段黑字。

### 驗收（NOK 實測）

- 視覺截圖（頁 1）確認：近天期徽章綠色圓角背景＋綠色圓點＋綠色文字，遠天期徽章黃色同款，均與 HTML 截圖語意色一致。
- 視覺截圖（頁 2）確認：「Call $7.6M」綠字、「Put $12.1M」紅字，與 HTML 顏色一致。
- 352 RSpec + 23 Python 測試全過。

### 教訓（累積、非首次）

Options Flow 面板與推薦分析區塊已連續被發現遺漏三次子區塊／樣式（名詞解釋圖卡 → Flow 總額與重疊提示 → 顏色與推薦徽章）。這是同一個根因的重複發作：**移植 HTML 到 PDF 時，只照抄「文字內容」，沒有把「視覺語意（顏色/徽章/色塊）」也當作要涵蓋的範圍**。往後任何 HTML→PDF 的移植工作，驗收前必須把來源 HTML 的 render 方法整份讀過一遍，列出全部視覺元素清單（文字、色彩、徽章、圖示），不能只看資料本身。

## 補做四（2026-07-08）：術語字卡加入 PDF（含 IPA 音標雙字型嵌入）

### 需求

使用者要求把術語字卡（15 張）也放進向量 PDF，正反面內容攤平合併顯示（不留白等翻面）；發現音標消失後，使用者進一步要求嵌入支援 IPA 音標的字型，而非拿掉音標。

### 問題：Noto Sans TC 不含 IPA Extensions 字元

首次實作把字卡文字塞進 PDF 後，`pdftotext` 逐字比對發現 IPA 音標大量缺字（`/liːps/` 變成 `/lips/`、`/ˈdɛltə/` 變成 `/dlt/`）——查證 Noto Sans TC 原始完整字型本身就不含 `ɪ ɛ ə ʊ ɡ ɑ ʃ ʌ ˈ ˌ ː` 等 IPA Extensions 區塊符號，這不是 subset 流程的錯，是字型設計範圍本身的限制。

### 解法：第二個字型嵌入 + 混合字型單行繪製

- 新字型：Google Fonts CDN **Noto Sans Regular**（拉丁字型，非 TC 版，version 2.015，OFL 1.1）。驗證涵蓋 `VOCAB_CARDS` 全部 15 個 IPA 音標用到的 31 個字元（100% 覆蓋）。
- `script/extract_ipa_charset.py`：程式化掃描 `VOCAB_CARDS` 的 `ipa` 欄位取得實際用字（不手工列舉）。
- `script/subset_ipa_font.sh`：完整字型 556,328 bytes → 子集 **5,024 bytes**（極小，只含 31 個音標符號）。
- JS `loadFontFile()` 重構為通用函式，兩個字型（主字型／IPA 字型）都各自 fetch+addFileToVFS+addFont+驗證，**任一字型失敗都中止匯出**——這條規則對兩個字型一視同仁，不只主字型。
- `renderVocabCards` 的標題行改成混合字型繪製：`pdf.setFont(FONT_ALIAS)` 畫英文 → `pdf.setFont(IPA_FONT_ALIAS)` 畫音標 → `pdf.setFont(FONT_ALIAS)` 畫中文，用 `pdf.getTextWidth()` 逐段計算並手動定位 x 座標（jsPDF 單次 `pdf.text()` 呼叫只能用單一字型，混合字型必須逐段切換）。

### 驗收（NOK 實測）

- `pdftotext` 逐字比對：原始碼 15 個 `ipa:` 欄位字串，**15/15 全數在 PDF 輸出中逐字命中**，0 豆腐字。
- **字型 404 中止測試擴及 IPA 字型**：攔截 fetch 讓 IPA 字型請求回 404，確認 `caughtError = "IPA 音標字型下載失敗（HTTP 404），已中止匯出"`、`pdf.save()` 未被呼叫——證明「任一字型失敗必炸」規則對兩個字型都生效，不是只驗證了主字型。
- 視覺截圖確認：LEAPS /liːps/、Strike Price /straɪk praɪs/、Delta /ˈdɛltə/、Open Interest /ˈoʊpən ˈɪntrəst/、Volume /ˈvɑːljuːm/ 等音標清晰完整、字型切換無縫、排版無破版。
- PDF 檔案大小：266,927 bytes（多了 15 張字卡 + 第二個字型後仍在合理範圍）。
- 352 RSpec + 23 Python 測試全過。

### 補充：字型子集內容更新後的重啟教訓（重複發生）

本次任務內先後兩次更新既有字型子集內容（第一次補新增說明文字的 4 個中文字，第二次新增第二個 IPA 字型檔案）。兩次都需要重啟 `fairprice-rails` 才能讓 propshaft 產生正確的新 digest——這是 `tasks/lessons.md` 已記錄過的教訓（migration／新增 asset 目錄後必須重啟），這次證實**同一份 asset 檔案內容變更（檔名不變）** 也適用同一條規則，不是只有新增檔案/目錄才要重啟。
